...
describe('paginator', () => {
  ...
  it('loads partner-dashboard app : checks paginator/items per page/next-previous pages', () => {
    ...
    cy.wait('@paginator').its('response.statusCode').should('equal', 200)
    cy.getElement('page-number-text').should('include.text', 'Page 2 of 2')
    cy.getElement('left-button').click()
    cy.getElement('page-number-text').should('include.text', 'Page 1 of 2')
